...
int mystrcopy(char *to ,char *from, int n)
{
	if(n < 0)
	{
		return -1;
...
ssize_t demo_read(struct file *filp,char __user *buffer,size_t size,loff_t *offlen)//读
{
	...
	mystrcopy(kbuf,kbuf+size,counter);
...
long demo_ioctl(struct file *filp,unsigned int cmd,unsigned long arg)
{
	...
	case CMDR:
		printk("CMDR OK!\n");
		if(copy_to_user((struct cmdbuf *)arg,&buf,sizeof(struct cmdbuf)) != 0)
		{
	...
	case CMDW:
		printk("CMDW OK!\n");
		if(copy_from_user(&buf,(struct cmdbuf *)arg,sizeof(struct cmdbuf)) != 0)
		{
